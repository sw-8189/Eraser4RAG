...
class PromptDataset(Dataset):
    def __init__(self, args, tokenizer, data):
        self.examples = []
        for record in tqdm(data, desc="Preparing rewrite prompts"):
...
def format_prompt(record, args):
    ...
    for context in record.get("ctxs", []):
        ...
        local_public.append(context.get("public", []))
        local_private.append(context.get("private", []))
    return prompts, local_public, local_private
...
def get_args(argv=None):
    parser = argparse.ArgumentParser(description="Rewrite the special privacy-erasure evaluation set.")
    parser.add_argument("--data_dir", "--data-dir", dest="data_dir", default="./dataset/special_data/popqa_10_25_special.jsonl")
...
def build_output_record(batch, item_index, rewritten_groups):
    return {
        ...
        "private": batch["privacy"][item_index],
    ...
    with torch.no_grad(), output_path.open("w", encoding="utf-8") as fout:
        for batch_index, batch in enumerate(loader):
            ...
            for prompts in batch["prompts"]:
                ...
                process_times.append(time.perf_counter() - start_time)
                rewritten_groups.append(tokenizer.batch_decode(outputs, skip_special_tokens=True))
            if process_times:
                LOGGER.info("Mean inference time per document group: %.4fs", fmean(process_times))
